...
import { CelulaForm, CelulaResponse, CelulasPagination, ListCelulasResponse } from '../interfaces/celula-admin.interface';
...
export class CelulaAdminService {
  ...
  private _celulasResponse = signal<ListCelulasResponse | null>(null);
  private _celulasList    = signal<CelulasPagination | null>(null);
  ...
  public celulas   = computed(() => this._celulasList());
  ...
  getAllcelulaes(page: number, term: string): Observable<Boolean> 
  {
    ...
    .pipe(
      map(({ status, celulas }) => {
        ...
        this._celulas.set(celulas.data);
