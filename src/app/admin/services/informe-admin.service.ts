...
import { InformeForm, InformeResponse, ListInformes, Report, Reports } from '../interfaces/informe-admin.interface';
...
export class InformeAdminService {
  ...
  private _informes   = signal<Reports | null>(null);
  ...
  getAllinformees(page: number, term: string): Observable<Boolean> 
  {
    ...
    return this.http.get<ListInformes>(url, {headers})
    .pipe(
      map(({ status, reports }) => {
