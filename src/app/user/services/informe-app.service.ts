...
import { InformeForm, InformeResponse, ListInformesResponse, Report, ReportsPagination } from '../interface/informe';

@Injectable({
  providedIn: 'root'
})
export class InformeAppService {
  ...
  {
    const url = `${this.baseUrl}/api/app_informe?page=${page}&term=${term}`;
  ...
  {
    const url = `${this.baseUrl}/api/app_informe/filtrar/informe?page=${page}&inicio=${inicio}&final=${final}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
  ...
  {
    const url = `${this.baseUrl}/api/app_informe/${id}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
  ...
  addInforme(informeForm: InformeForm): Observable<boolean>
  {
    ...
    const url = `${this.baseUrl}/api/app_informe`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
  ...
  {
    const body = informeForm;
    const url = `${this.baseUrl}/api/app_informe/update/${id}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
