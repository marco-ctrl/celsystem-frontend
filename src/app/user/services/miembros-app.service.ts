...
import { Asistente, ListAsistentes, ListMemberResponse, MemberResponse, MembersPagination, Miembro, Visita } from '../../admin/interfaces/miembro-admin.interface';
...
export class MiembrosService {
  ...
  getAsistente(celulaId: number, term: string){
    const url = `${this.baseUrl}/api/app_member/${celulaId}/asistencia?term=${term}`;
  ...
  getVisita(celulaId: number, term: string){
    const url = `${this.baseUrl}/api/app_member/${celulaId}/visita?term=${term}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
  ...
  {
    const url = `${this.baseUrl}/api/app_member?page=${page}&term=${term}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
  ...
  addMiembro(memberForm: Miembro): Observable<boolean>
  {
    ...
    const url = `${this.baseUrl}/api/app_member`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
  ...
  {
    const body = memberForm;
    const url = `${this.baseUrl}/api/app_member/${id}`;
    const token = localStorage.getItem('token');
    const headers = new HttpHeaders()
      .set('Authorization', `Bearer ${token}`);
